 ...
 *
 * ROOT CAUSE THIS FIXES (usability): the old panel presented roughly forty
 ...
 * list, so people changed MTU or fragment sizes at random, could not get back
 * to a working configuration, and reported the tunnel as broken. Nothing said
 * which controls matter, and nothing said the defaults are usually right.
 *
 * So the surface is now progressive:
 *
 *   Essentials  the ten or so controls that actually change whether a
 *               connection succeeds on a filtered network.
 *   Everything  the full engine surface, unchanged in capability.
 *
 * Every field still writes to exactly the same ConnectionProfile properties
 ...
 * surface is a destination now, so it is always open.
 */
@Composable
fun AdvancedPanel(
    ...
    var depth by remember { mutableStateOf(Depth.BASIC) }
    ...
    Column(modifier = modifier.fillMaxWidth()) {
        SectionRule(label = stringResource(R.string.advanced), topSpace = 22)
        ...
        SectionRule(label = stringResource(R.string.section_connection))
        ...
        SectionRule(label = stringResource(R.string.section_routing))
        ...
        if (full) {
            SectionRule(label = stringResource(R.string.section_transport))
            ...
            SectionRule(label = stringResource(R.string.section_routes))
            ...
            SectionRule(label = stringResource(R.string.section_zerotrust))
            ...
            if (profile.teamAuth != TeamAuth.OFF) {
                ...
                when (profile.teamAuth) {
                    TeamAuth.SERVICE_TOKEN -> {
                        ...
                        Spacer(Modifier.height(12.dp))
                        // Masked: an organization credential must not be readable
                        // over a shoulder or land in a screenshot.
                        LtrOutlinedTextField(
            ...
            SectionRule(label = stringResource(R.string.section_security))
            ...
            SectionRule(label = stringResource(R.string.section_engine_tuning))
        ...
        SectionRule(label = stringResource(R.string.section_reset))
        ActionPill(
